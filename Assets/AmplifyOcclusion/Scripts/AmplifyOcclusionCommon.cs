...
public static class AmplifyOcclusionCommon
{
	...
	public static Material CreateMaterialFromShader( Shader shader, bool throughErrorMsg )
	{
		if( shader == null )
		{
			if( throughErrorMsg == true )
			{
				Debug.LogErrorFormat( "[AmplifyOcclusion] Cannot find shader: \"{0}\". Please contact support@amplify.pt", shader.name );
			}
			return null;
		}
